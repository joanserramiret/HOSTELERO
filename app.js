...
function aplicarMaster(){
  ...
  DB.productos=(m.productos||[]).map(function(p){
    var prod={id:p.id,nombre:p.nombre,categoriaId:p.categoriaId,precio:p.precio||0,ic:p.ic||'🍽️',stock:(p.stock==null?100:p.stock),controlStock:(p.controlStock!==false)&&!p.esMenu,activo:p.activo!==false,pasePorDefecto:p.pasePorDefecto||'Primeros'};
    if(p.mods)prod.mods=p.mods;
  ...
  if(m.mesas&&m.mesas.length){
    ...
    DB.salas=(m.salas||[]).map(function(s,i){return {id:s.id,nombre:s.nombre,orden:i+1};});
    DB.mesas=m.mesas.map(function(mm,i){var pv=prev[mm.id]||{};return {id:mm.id,salaId:mm.salaId,nombre:mm.nombre,plazas:mm.plazas||4,forma:mm.forma||'cuadrada',estado:pv.estado||'libre',ticketId:pv.ticketId||null,x:(pv.x==null?((i%4)*156+24):pv.x),y:(pv.y==null?(Math.floor(i/4)*140+24):pv.y)};});
  }
  save();return true;
}
