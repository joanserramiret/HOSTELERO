...
function masterSeed() {
  ...
  return { categorias: cats, productos: prods, salas: salas, mesas: mesas, usuarios: usuarios };
}

function cargar() {
  var s; try { s = JSON.parse(fs.readFileSync(DATAFILE, 'utf8')); } catch (e) { s = {}; }
  if (!s.comandas) s.comandas = [];
  if (!s.master || !s.master.productos || !s.master.productos.length) s.master = masterSeed();
